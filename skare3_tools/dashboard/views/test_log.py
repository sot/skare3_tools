...
import os
import re
from skare3_tools import dashboard, test_results as tr, config
...
def test_log(path):
    ...
    if not test_runs:
        return dashboard.get_template('error.html').render(
            title="404 Error",
            message="Run {run_id} not found".format(run_id=run_id))
    filename = os.path.join(config.CONFIG['data_dir'], 'test_logs',
                            test_runs[0]['run_info']['destination'],
                            filename)
    if not os.path.exists(filename) or not os.path.isfile(filename):
        return dashboard.get_template('error.html').render(
            title="404 Error",
            message="File {path} not found".format(path=path))
