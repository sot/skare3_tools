...
def test_stream():
    ...
    test_suite_names = sorted(set(sum([[k for k in t['test_suites'].keys()] for t in test_runs], [])))
    test_suites = []
    column_names = [str(i) for i in range(len(test_runs))]
    for ts_name in test_suite_names:
        test_cases = []
        test_case_names = sorted(set(sum([list(t['test_suites'][ts_name]['test_cases'].keys())
                                          for t in test_runs if ts_name in t['test_suites']], [])))
        for tc_name in test_case_names:
            row = [
                ('name', tc_name)
            ]
            row += [(n, (t['test_suites'][ts_name]['test_cases'][tc_name]['status']
                    if ts_name in t['test_suites'] and
                       tc_name in t['test_suites'][ts_name]['test_cases'] else 'skipped'))
