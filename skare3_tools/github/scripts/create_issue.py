...
def parser():
    ...
    parse.add_argument(
        "--title", help="Issue Title. Ignored if --latest-release is given"
    )
    parse.add_argument(
        "--body", help="Issue Description. Ignored if --latest-release is given"
    )
...
def main():
    ...
    if args.latest_release:
        ...
        ok_or_exit(
            release,
            f"Failed to create issue after latest release of {args.latest_release}",
        )
